...
def smoothedGradient(im,sigma):
    ...
    im_x, im_y = gradient(smoothGaussian(im, sigma))
...
def HarrisScore(im,sigma1,sigma2,k=0.06):   
    ...
    im_x, im_y = smoothedGradient(im, sigma1)
    m_11 = smoothGaussian(im_x**2, sigma2)
    m_12 = smoothGaussian(im_x*im_y, sigma2)
    m_21 = smoothGaussian(im_x*im_y, sigma2)
    m_22 = smoothGaussian(im_y**2, sigma2)
    
    R = m_11*m_22 - m_12*m_21 - k*((m_11 + m_22)**2)
...
def SSDTable(patches1,patches2):
    ...
    for i in range(patches1.shape[0]):
        for j in range(patches2.shape[0]):
			table[i, j] = np.sum((patches1[i] - patches2[j])**2)
    return table
...
def main():
    ...
    print("#################")
    print("Smoothed Gradient")
	
    im_x,im_y=smoothedGradient(im1,sigma=2) 

    print(im_x[200:205,300:302])
