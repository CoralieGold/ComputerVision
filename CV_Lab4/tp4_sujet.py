...
def distancePointsLine(points, rho, theta):
    ...
    distance = np.abs(points[:, 0] * cos(theta) + points[:, 1] * sin(theta) - rho)
...
def countInliersLine(points, rho, theta, tau):
    ...
    #TODO code this function
    inliers_count = np.sum(distancePointsLine(points, rho, theta) < tau)
...
def h(x,tau):    
    ...
    for i in range(len(x)):
		hx[i] = (1 - (x[i]/tau)**2)**3 if np.fabs(x[i]) < tau else 0
    return hx 
...
def smoothLineScore(points, rho,theta,tau):
    ...
    smooth_score = np.sum(h(distancePointsLine(points, rho, theta), tau))
...
def getRhos(points,theta):
    ...
    for i in range(len(points)):
		rhos[i] = points[i][0] * cos(theta) + points[i][1] * sin(theta)
...
def main():
    ...
    print("##########################")
    print("Distance Points Line")
    print(distancePointsLine(points[1:5,:], 2,0.3))
    #array([ 0.02298727,  0.60614286,  0.87389925,  0.04036134])
    
    print("##########################")
    print("Count In Liers Line")
    print(countInliersLine(points,  2,0.3,tau)) # 68
    print("h")
    print(h(np.linspace(0,1,11),0.5))
    #array([ 1.      ,  0.884736,  0.592704,  0.262144,  0.046656,  0.      ,
    #   -0.      , -0.      , -0.      , -0.      , -0.      ])
    
    print("##########################")
    print("Smooth Line Score")
    print(smoothLineScore(points, 2,0.3,tau)) # 35.398607765521561
    print("Get Rhos")
    print(getRhos(points[1:5,:],0.3))
    #array([ 2.02298727,  2.60614286,  2.87389925,  2.04036134])
   
    print("##########################")
    print("Score Brute Force")
