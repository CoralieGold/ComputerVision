...
def computeNormalizedImage(im,display_distribution=False):
	...
	im_intensity_normalized = np.zeros(im.shape)
	im_intensity_normalized[ :, :, 0] = im[:, :, 0] / np.linalg.norm(im, axis=(2))
	im_intensity_normalized[ :, :, 1] = im[:, :, 1] / np.linalg.norm(im, axis=(2))
	im_intensity_normalized[ :, :, 2] = im[:, :, 2] / np.linalg.norm(im, axis=(2))
...
def imageKmeans(im_intensity_normalized,nb_clusters):
	...
	point_colors = im_intensity_normalized.reshape((im_intensity_normalized.shape[0] * im_intensity_normalized.shape[1], 3))
	means, imlabels = kmeans2(data=point_colors, k=nb_clusters)
